...
def build_prompt(snapshot: dict) -> str:
  ...
  - devices
  - recent events
  - a 'mode' field inferred as: 'anchor', 'underway', or 'in_port'
  - an 'ai_mode' field describing how much autonomy you have:
      - 'monitor'
      - 'assist'
      - 'autonomous'
  """
  ai_mode = str(snapshot.get("ai_mode", "assist"))

  # Presence semantics the UI uses:
  # - Onboard     -> ai_mode='assist'  (skipper onboard)
  # - Unattended  -> ai_mode='autonomous' (nobody onboard, AI in full control)
  mode_section = (
    ...
    "- The JSON will also include an 'ai_mode' field describing how much autonomy you have.\n"
    f"  The current ai_mode is '{ai_mode}'.\n"
    "- ai_mode='monitor': you mostly observe and should normally output a single 'no_op' "
    "unless there is a very clear safety problem.\n"
    "- ai_mode='assist': the skipper is onboard. Focus on safety (navigation/anchor lights, "
    "dangerous conditions) and avoid micro-managing simple interior/comfort lights.\n"
    "- ai_mode='autonomous': the vessel is unattended. You must take full control of all "
    "devices whose 'ai_control' flag is enabled, to keep the yacht safe, watertight, secure "
    "and energy-efficient.\n"
  )

  if ai_mode == "autonomous":
    control_section = (
      "- You may control any device whose 'ai_control' flag is true. "
      "Do not change devices where 'ai_control' is false.\n"
      "- Prioritise: (1) safety and watertight integrity, (2) security and alarms, "
      "(3) energy efficiency, (4) comfort if it does not conflict with the above.\n"
    )
  else:
    # monitor / assist – skipper is around
    control_section = (
      "- Only touch devices and scenes that are clearly related to safety or "
      "navigation/anchor lights.\n"
      "- Assume the skipper will handle simple interior or comfort lighting unless there is "
      "a strong safety reason to act.\n"
    )
  ...
  other_rules = (
    ...
    "- You may still suggest actions for lighting and other safety-related devices.\n"
  )

  return mode_section + control_section + other_rules
...
def enforce_nav_light_rules(snapshot: dict, payload: dict) -> dict:
  """
  Hard COLREGs rules around anchor_light and nav_lights, on top of
  whatever the model suggests.

  - Uses inferred mode from current snapshot.
  - Only acts on lights where ai_control == True.
  - Avoids fighting manual control if ai_control is False.
  """
  devices = snapshot.get("devices", [])
  actions = payload.get("actions", [])
...
def build_human_explanation(snapshot: dict, command_payload: dict) -> str:
  ...
  devices = snapshot.get("devices", [])
  mode = infer_mode(snapshot)
  ai_mode = snapshot.get("ai_mode", "assist")
  ...
  lines: List[str] = []
  lines.append(f"Mode: {mode}. AI mode: {ai_mode}.")
  lines.append(
    f"Bilge high float: {on(bilge_high)}, bilge auto override: {on(bilge_override)}."
  )
  lines.append(f"Anchor light: {on(anchor)}, nav lights: {on(nav)}.")
...
async def send_ai_log(
  ...
  payload = {
    "generated_at": command_payload.get("generated_at")
...
def apply_hard_rules(snapshot: dict, payload: dict) -> dict:
  ...
  bilge_high = next(
    (d for d in devices if d.get("id") == "bilge_float_high"), None
  )
  bilge_override = next(
    (d for d in devices if d.get("id") == "bilge_pump_auto_override"), None
  )

  # Use latched state for high bilge: stays True for 60s after last True
  bilge_high_state = get_sensor_state_with_latch(
    "bilge_float_high", bilge_high, hold_seconds=60
  )
  ...
  if (
    ...
    in_rest_window = (
      bilge_forced_rest_until is not None and now < bilge_forced_rest_until
    )
      ...
      actions.append(
        {
          ...
          "reason": (
            "Bilge high float is active and override is OFF – forcing pump ON for safety."
          ),
        }
      )
    ...
    actions.append(
      {
        ...
        "constraints": {},
        "reason": (
          "Bilge high float cleared after AI-driven pump use – turning bilge pump override OFF."
        ),
      }
    )
...
async def call_model(snapshot: dict) -> dict:
  ...
  # Add inferred mode into the JSON that the model sees
  mode = infer_mode(snapshot)
